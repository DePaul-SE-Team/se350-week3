...
4. Suppose a new weapon type needs its own behavior (e.g., a `Bow` that can miss, unlike a `Sword`). What has to change, and where, to support that with the current design?
5. Refactor `Player` so a `Weapon` is passed into its constructor instead of being created inside it. Show the before/after code for `Player`'s constructor and for `Entry.main`.
